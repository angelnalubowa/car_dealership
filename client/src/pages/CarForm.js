...
const CarManagementForms = ({ activeForm = "cars", onCancel }) => {
  const { updateListings } = useListings(); // Access the context
  const [formData, setFormData] = useState({});
  const navigate = useNavigate();

  // Debugging: Ensure activeForm is received correctly
  ...
  const renderDynamicForm = () => {
    if (!activeForm || !formSchema[activeForm]) {
      return <p style={{ color: "red" }}>Invalid form selection.</p>;
    }
      ...
      <Card className="form-card">
        <CardContainer>
          <h2 className="form-title">
            {activeForm ? activeForm.charAt(0).toUpperCase() + activeForm.slice(1) : "Form"}
          </h2>
          <Form
            layout="horizontal"
            className="main-form"
            onFinish={() => handleSubmit(`/${activeForm}`, activeForm)}
          >
            {renderDynamicForm()}
